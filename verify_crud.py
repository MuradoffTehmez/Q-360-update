...
def run_tests():
    ...
    print(f"[Admin POST] Status: {res.status_code}")
    ...
    client.force_login(user1)
    res = client.post('/api/v1/feedback/public-recognitions/', {'feedback': feedback.id}, content_type='application/json')
    print(f"[Sender POST] Status: {res.status_code}, Response: {res.json()}")
    if res.status_code == 201:
        rec_id = res.json().get('id')
        print(f"DB Check: PublicRecognition exists? {PublicRecognition.objects.filter(id=rec_id).exists()}")
        
        # PUT as sender
        res = client.patch(f'/api/v1/feedback/public-recognitions/{rec_id}/', {'is_featured': True}, content_type='application/json')
        print(f"[Sender PATCH] Status: {res.status_code}")
        
        # Unauthorized DELETE
        client.force_login(admin)
        res = client.delete(f'/api/v1/feedback/public-recognitions/{rec_id}/')
    ...
    # Badges POST
    client.force_login(admin)
    res = client.post('/api/v1/engagement/badges/', {'name': 'Super Star', 'description': 'Test badge', 'icon_class': 'star'}, content_type='application/json')
    print(f"[Admin POST Badge] Status: {res.status_code}")
    badge_id = res.json().get('id')
    
    # UserBadge POST
    res = client.post('/api/v1/engagement/user-badges/', {'user': user1.id, 'badge': badge_id}, content_type='application/json')
    print(f"[Admin POST UserBadge] Status: {res.status_code}")
    ub_id = res.json().get('id')
    ...
    session = ReviewSession.objects.create(manager=admin, employee=user1, status='scheduled')
